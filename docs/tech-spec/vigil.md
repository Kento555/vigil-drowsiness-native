...
| BR-006 | Alertness scoring | A completed drive's alertness score is `max(0, min(100, 100 − alarms×15 − drowsy×8))`. | Drive ends (`stopWatching`) | Score displayed on Drive Summary and History screens | [SHIPPED] `lib/history.ts` `scoreFor()` |
